...
export async function pushTrackChanges(userId: string, addedTrackItems: CollectionItem[], deletedTrackIds: string[]): Promise<void> {
  ...
  if (!response.ok) {
    ...
    console.error("Track update failed. Response:", errorText);
    throw new Error(`Track update failed: ${response.status} ${response.statusText} - ${errorText}`);
  }
}
...
export async function pushBulkTrackChanges(userId: string, addedTrackItems: CollectionItem[]): Promise<void> {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error("Bulk track update failed. Response:", errorText);
    throw new Error(`Bulk track update failed: ${response.status} ${response.statusText} - ${errorText}`);
  }
}
...
export async function pushImmutableContent(data: Collection | CollectionItem[]): Promise<number> {
  ...
  if (response.status !== 201) {
    const errorText = await response.text();
    console.error("Immutable content push failed. Response:", errorText);
    throw new Error(`Immutable content push failed: ${response.status} ${response.statusText} - ${errorText}`);
  }
